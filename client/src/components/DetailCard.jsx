  ...
  return (
    ...
    textShadow: '0 0 #004e64', backgroundColor: '#fefefe'}}>
        ...
        <Stack flexDirection='column' sx={{py: '0.5rem', px:'1rem', width: '65%'}}>
            ...
            <Typography variant='body1' sx={{color: '#7400b8'}}>ID: <span style={{color: '#004e64', fontWeight: 'bold'}}>{String(_id).slice(0,3)}</span></Typography>
            ...
            {pathogensData.map((pathogen, indx) => (
                <Box key={indx}>
                <Typography variant='body1'sx={{color: '#7400b8'}}>{pathogen.pathogensName} : {pathogen.infected? <span style={{color: 'darkred', marginLeft: '.3rem'}}>Infected</span> : <span style={{color: 'darkgreen', marginLeft: '.3rem'}}>Not Infected</span>} </Typography>
                </Box>
            ))}
            </Stack>
        </Stack>
